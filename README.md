...
- 会话记录导航：支持在会话中快速定位并跳转到特定消息，替代旧版搜索界面。
- 会话克隆与摘要创建：支持克隆既有会话，并基于会话摘要创建新会话。
- 旧版数据迁移与导入：支持检测旧版工作区数据，并提供手动迁移、导入预览、分组展示与空状态处理。
- 应用变体配置与构建：支持自定义应用标识、名称、配置目录、自动更新策略与变体构建流程。
- 工作区运行时数据目录重构：将运行时数据迁移至 `.craft-agents` 目录，并支持自动迁移与备份。
- 工作区配置路径统一：统一工作区配置与插件清单存储路径，降低路径分叉导致的数据不一致风险。
- 消息网关变体适配：合入上游 WhatsApp / Telegram 消息网关能力，并将运行时消息目录接入 Crystal workspace data path。
- 会话工具元数据保护：保留本地工具元数据校验包装，并覆盖上游新增 inter-session 与 messaging gateway 工具。
- 工具验证与调试日志：记录主进程工具验证失败、Pi Agent 自动压缩调试信息与原生工具输入验证日志。
- 聊天渲染稳定性：过滤无渲染内容的活动项，并修复切换工作区时滚动位置恢复异常。
- 专注模式增强：支持悬浮侧栏延迟设置与活动栏容器高度选项。
- 桌面构建与 CI 增强：新增桌面构建工作流、版本化产物命名、发布参数控制与多平台产物上传优化。
