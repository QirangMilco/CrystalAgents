# Crystal Agents Windows Installer
...
$DOWNLOAD_DIR = "$env:TEMP\crystal-agent-install"
$APP_NAME = "Crystal Agents"
...
if (-not $filename) {
    $filename = "Crystal-Agents-$version-$arch.exe"
}
...
$process = Get-Process -Name "Crystal Agents" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing Crystal Agents..."
...
Write-Info "Adding 'crystal-agents' command to PATH..."

$binDir = "$env:LOCALAPPDATA\Crystal Agents\bin"
$cmdFile = "$binDir\crystal-agents.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\Crystal Agents\Crystal Agents.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'crystal-agents' command)"
} else {
    Write-Success "Command 'crystal-agents' is ready"
}
...
Write-Host ""
Write-Host "  Crystal Agents has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: crystal-agents (restart terminal first)"
Write-Host ""
}
