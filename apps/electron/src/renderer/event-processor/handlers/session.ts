...
export function handleInfo(
  ...
  // If this is a compaction complete, update the existing compacting status message.
  // If no compacting status exists (e.g. auto-compaction events arrived before the
  // status message was processed), add the compaction card as a new info message.
  if (event.statusType === 'compaction_complete') {
    ...
    const updatedMessages = hasCompactingStatus
      ? session.messages.map(m =>
          m.role === 'status' && m.statusType === 'compacting'
            ? compactionInfo
            : m
        )
      : [...session.messages, compactionInfo]
    return {
      state: {
        session: {
          ...session,
