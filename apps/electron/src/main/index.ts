...
import { getDefaultWorkspacesDir, getWorkspaceDataPath, WORKSPACE_DATA_DIR } from '@craft-agent/shared/workspaces'
...
// and PATH prepend. uv auto-downloads Python 3.12 on first use (~5s, then cached).
{
  ...
  mainLog.info('[debug-env] Tool debug flags:', {
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            getMessagingDir: (wsId: string) => {
              ...
              const rootPath = ws?.rootPath ?? join(getDefaultWorkspacesDir(), wsId)
              return join(getWorkspaceDataPath(rootPath), 'messaging')
            },
            ...
            // in the variant-aware Electron logs directory.
