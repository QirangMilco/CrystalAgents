...
# 6. Build and stage subprocess servers (session-mcp-server / pi-agent-server)
echo "Building and staging subprocess servers..."
cd "$ROOT_DIR"
bun run scripts/build/stage-subprocess-servers.ts --platform darwin --arch "$ARCH"
...
# 7a. Resolve the target arch's binary package. If the host arch matches the
...
# 8. Copy ripgrep (was previously bundled inside the SDK at vendor/ripgrep/;
...
# 9. Copy network interceptor sources.
